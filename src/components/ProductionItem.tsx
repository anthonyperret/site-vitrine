    ...
    <Link href={production.url} className="flex-1">
      ...
      >
        ...
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 transition-all duration-300 flex flex-col items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
          <h3 className="font-bebas-neue text-2xl uppercase text-ivoire">{production.title}</h3>
          <p className="font-dm-sans text-xs text-cuivre tagline">{production.tagline}</p>
        </div>
      </div>
    </Link>
  )
